...
(define mupl-map 
  (fun "fname" "fpar" 
       (fun "m" "lst" 
            (ifaunit(var "lst") 
                    (aunit)
                    (apair (call (var "fpar") (fst (var "lst")))    ;(call (fpar (car lst)))
                           (call (var "m") (snd (var "lst"))))))));(call (m (cdr lst)))

(define mupl-mapAddN 
  (mlet "map" mupl-map  ;map = mupl-map
        (fun #f "x" (call (var "map") (fun #f "y" (add (var "x") (var "y")))))))  ;x:int y:list return:map y list => x+y list
